...
export function ProjectOptimizer({ resumeId, jobId, projects, onApply }: ProjectOptimizerProps) {
  ...
  const handleGenerateReplacements = async () => {
    ...
    for (const idx of indices) {
      ...
      try {
        ...
        const msg =
          raw.toLowerCase().includes('rate limit') || raw.includes('429')
  ...
  const handleRegenerate = async (idx: number) => {
    ...
    try {
      ...
      const msg2 =
        raw2.toLowerCase().includes('rate limit') || raw2.includes('429')
          ? 'Rate limit — wait 30s and retry'
          : raw2.length > 120
            ? 'Generation failed. Retry.'
            : raw2;
                ...
                setReplacements(
                  new Map([
                    [
                      ...
                      {
                        projectIndex: -1,
                        generated: proj as ReplacementProject,
                        generating: false,
                        error: null,
                        accepted: null,
                      },
            ...
            (() => {
              ...
              return (
                <div className="border-2 border-black p-3 space-y-2 bg-white">
                  <div className="font-mono text-xs font-bold uppercase text-ink-soft">
                    Generated Project
                  </div>
                  <div className="font-semibold">{r.generated.name}</div>
                  {r.generated.role && (
                    <div className="text-xs text-ink-soft">{r.generated.role}</div>
                  )}
      ...
      <div className="p-4 space-y-4">
        ...
        {step === 'idle' && (
          <>
            <p className="font-mono text-xs text-ink-soft leading-relaxed">
              Checks each project&apos;s relevance to the JD. Low-scoring projects (priority to
              replace) are highlighted so you can generate JD-tailored replacements. Max 3 changes.
            </p>
        ...
        {step === 'results' && (
          <>
                    ...
                    >
                      <div className="p-3">
                        <div className="flex items-start gap-3">
                          {/* Score badge */}
                          <div
                            className={`shrink-0 w-10 h-10 flex flex-col items-center justify-center border ${scoreBg(item.relevance_score)}`}
                          >
                            <span
                              className={`font-mono text-sm font-bold leading-none ${scoreColor(item.relevance_score)}`}
                            >
                              {item.relevance_score}
                            </span>
                            <span className="font-mono text-[8px] text-ink-soft uppercase">
                              /100
                            </span>
                          </div>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center justify-between gap-2">
                              ...
                              <span
                                className={`shrink-0 text-[10px] font-mono font-bold uppercase px-1.5 py-0.5 border ${item.verdict === 'keep' ? 'bg-green-50 border-green-300 text-green-700' : 'bg-red-50 border-red-300 text-red-600'}`}
                              >
                            ...
                            <div className="mt-1.5 h-1.5 bg-gray-200 rounded-sm overflow-hidden">
                              <div
                                className={`h-full rounded-sm ${scoreBar(item.relevance_score)}`}
                                ...
                                {item.jd_skills_matched.length > 0 && (
                                  <div className="flex flex-wrap gap-1">
                                    ...
                                    {item.jd_skills_matched.map((s, i) => (
                                      <span
                                        key={i}
                                        className="bg-green-100 text-green-800 border border-green-300 px-1.5 py-0.5 rounded-sm font-mono"
                                      >
                                        {s}
                                      </span>
                                    ))}
                                ...
                                {item.jd_skills_missing.length > 0 && (
                                  <div className="flex flex-wrap gap-1">
                                    ...
                                    {item.jd_skills_missing.map((s, i) => (
                                      <span
                                        key={i}
                                        className="bg-red-100 text-red-800 border border-red-300 px-1.5 py-0.5 rounded-sm font-mono"
                                      >
                                        {s}
                                      </span>
                                    ))}
            ...
            <div className="border-t border-steel-grey pt-3 space-y-2">
              <p className="font-mono text-xs text-ink-soft">
                Selected to replace:{' '}
                <strong className="text-purple-700">{selectedToReplace.size}</strong> / 3 max
              </p>
              <Button
                className="w-full"
                size="sm"
              ...
              >
                Generate {selectedToReplace.size} Replacement
                {selectedToReplace.size !== 1 ? 's' : ''} →
              </Button>
            </div>
          </>
        )}
        ...
        {(step === 'replacing' || step === 'preview') && (
          <div className="space-y-4">
            <p className="font-mono text-xs text-ink-soft">
              {step === 'replacing'
                ? 'Generating tailored replacements...'
                : 'Review each replacement. Accept or reject before applying.'}
            </p>
            ...
            {Array.from(replacements.entries()).map(([idx, r]) => {
              ...
              return (
                <div
                  key={idx}
                  className={`border-2 ${r.accepted === true ? 'border-green-600' : r.accepted === false ? 'border-steel-grey opacity-60' : 'border-black'} bg-white`}
                >
                  <div className="px-3 py-2 border-b border-steel-grey bg-secondary flex items-center justify-between">
                    <div>
                      ...
                      <span className="ml-2 text-xs text-ink-soft">
                        {orig?.name ?? `Project #${idx + 1}`}
                      </span>
                      {analysisItem && (
                        <span
                          className={`ml-2 font-mono text-xs font-bold ${scoreColor(analysisItem.relevance_score)}`}
                        >
                    ...
                    {r.accepted === true && (
                      <span className="text-xs font-mono font-bold text-green-700">✓ Accepted</span>
                    )}
                    {r.accepted === false && (
                      <span className="text-xs font-mono text-steel-grey">✗ Rejected</span>
                    )}
                  </div>
                    ...
                    {r.error && (
                      <div className="text-xs text-red-600 font-mono bg-red-50 border border-red-200 p-2">
                        ...
                        <Button
                          ...
                          className="ml-2 h-5 text-xs rounded-none border-red-400"
                        >
                          Retry
                        </Button>
                      </div>
                        ...
                        <div>
                          ...
                          {r.generated.role && (
                            <div className="text-xs text-ink-soft">
                              {r.generated.role} {r.generated.years && `· ${r.generated.years}`}
                            </div>
                          )}
                        </div>
                        ...
                        {r.accepted === null && (
                          <div className="flex gap-2 pt-1">
                            <Button
                              ...
                              variant="outline"
                              onClick={() => handleRegenerate(idx)}
                              className="rounded-none border-black h-7 text-xs flex-none"
                            >
                            ...
                            <Button
                              ...
                              className="rounded-none border-black h-7 text-xs flex-1 text-red-600 hover:bg-red-50"
                            >
                            ...
                            <Button
                              ...
                              className="rounded-none h-7 text-xs flex-1 bg-green-700 hover:bg-green-800 text-white"
                            >
            ...
            {step === 'preview' && allDecided && acceptedCount > 0 && (
              <div className="border-t border-steel-grey pt-3 space-y-2">
                <p className="font-mono text-xs text-ink-soft">
                  <strong className="text-green-700">{acceptedCount}</strong> replacement
                  {acceptedCount !== 1 ? 's' : ''} accepted. Apply to resume?
                </p>
            ...
            {step === 'preview' && allDecided && acceptedCount === 0 && (
              <div className="border-t border-steel-grey pt-3">
                <p className="font-mono text-xs text-ink-soft mb-2">
                  All rejected. Go back to re-select?
                </p>
                <Button
                  variant="outline"
                  size="sm"
